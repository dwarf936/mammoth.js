...
function BodyReader(options) {
    ...
    var xmlElementReaders = {
        ...
        "w:oMath": function(element) {
            try {
                var convertOmmlToMathml = require('./convert-omml-to-mathml');
                var convertMathmlToLatex = require('./convert-mathml-to-latex');
                var omml = xml.writeString(element);
                var mathml = convertOmmlToMathml(omml);
                var latex = convertMathmlToLatex(mathml);
                return elementResult(new documents.Math(latex, {type: "display"}));
            } catch (e) {
                console.error("Error converting math formula:", e);
                return elementResult(new documents.Text("[Math formula conversion failed]"));
            }
        },
