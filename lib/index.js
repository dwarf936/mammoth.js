...
function getUserStyle(input, styleName) {
    return unzip.openZip(input)
        ...
        .then(function(documentResult) {
            if (!documentResult.value || !documentResult.value.styles) {
                return null;
            }
            
            var styles = documentResult.value.styles;
            
            // Try to find by style ID first
            var styleById = styles.findStyleById(styleName);
            if (styleById) {
                return styleById;
            }
            
            // Try to find by style name
            var allStyles = styles.getAllStyles();
            for (var styleId in allStyles) {
                var style = allStyles[styleId];
