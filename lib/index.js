...
function getUserStyle(input, styleName, options) {
    ...
    return unzip.openZip(input)
        .then(function(docxFile) {
            return readStylesFromDocx(docxFile, options)
                .then(function(styles) {
                    var style = styles.findStyleByName(styleName);
                    if (style) {
                        // Return a clean object with only the style properties
                        return {
                            name: style.name,
                            type: style.type,
...
function getAllStyles(input, options) {
    options = readOptions(options);
    ...
    return unzip.openZip(input)
        .then(function(docxFile) {
            return readStylesFromDocx(docxFile, options)
                .then(function(styles) {
                    return styles.getAllStyles().map(function(style) {
                        return {
                            name: style.name,
                            type: style.type,
                            fontSize: style.fontSize,
...
function readStylesFromDocx(docxFile, options) {
    var docxReader = require("./docx/docx-reader");
...
exports.styleMapping = function() {
    ...
    var message = 'Use a raw string instead of mammoth.styleMapping e.g. "p[style-name=';
    message += quote + 'Title' + quote + '] => h1" instead of mammoth.styleMapping("p[style-name=';
    message += quote + 'Title' + quote + '] => h1")';
