...
function markdownWriter() {
    ...
    function close(tagName) {
        ...
        // Add header separator row after closing first tr (whether in thead or not)
        if (tagName === "tr") {
            var parentTag = null;
            if (elementStack.length > 0) {
                parentTag = elementStack[elementStack.length - 1].tagName;
            }
            // Check if parent is table, thead, or tbody and if this is the first row
            var currentParentElement = elementStack[elementStack.length - 1];
            if (currentParentElement && (parentTag === "table" || parentTag === "thead" || parentTag === "tbody")) {
                // Check if this is the first child row
                if (currentParentElement.children && currentParentElement.children.length === 1) {
                    // Always add separator row for first row of table
                    var headerSeparator = "| ";
