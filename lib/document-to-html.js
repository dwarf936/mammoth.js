...
var lowcodeWriter = require("./writers/lowcode-writer");
...
function DocumentConverter(options) {
    return {
        ...
        convertToLowcode: function(element) {
            var comments = _.indexBy(
                element.type === documents.types.document ? element.comments : [],
                "commentId"
            );
            var conversion = new DocumentConversion(options, comments);
            return conversion.convertToLowcode(element);
        }
    };
}
...
function DocumentConversion(options, comments) {
    ...
    function convertToLowcode(document) {
        var messages = [];
        ...
        return promises.mapSeries(deferredNodes, function(deferred) {
            ...
            var lowcodeWriterInstance = lowcodeWriter();
            Html.write(lowcodeWriterInstance, Html.simplify(replaceDeferred(html)));
            return new results.Result(lowcodeWriterInstance.asSchema(), messages);
        });
    }
    ...
    function convert(document) {
        ...
        return promises.mapSeries(deferredNodes, function(deferred) {
            ...
            if (options.outputFormat === "lowcode") {
                var lowcodeWriterInstance = lowcodeWriter();
                Html.write(lowcodeWriterInstance, Html.simplify(replaceDeferred(html)));
                return new results.Result(lowcodeWriterInstance.asSchema(), messages);
            } else {
    ...
    return {
        ...
        convertToLowcode: convertToLowcode
    };
}
